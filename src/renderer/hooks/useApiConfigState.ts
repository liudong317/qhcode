...
export function useApiConfigState(options: UseApiConfigStateOptions = {}) {
  ...
  const applyQinghongRelay = useCallback(() => {
    const nextProfileKey = profileKeyFromProvider('custom', 'openai');
    ...
    dispatch({
      ...
      updater: (current) => ({
        ...current,
        baseUrl: QINGHONG_RELAY_BASE_URL,
        useCustomModel: true,
        customModel: current.customModel?.trim() || QINGHONG_RELAY_DEFAULT_MODEL,
        model: current.model?.trim() || QINGHONG_RELAY_DEFAULT_MODEL,
      }),
    });
    dispatch({ type: 'SET_ACTIVE_PROFILE_KEY', payload: nextProfileKey });
