...
import {
  QINGHONG_RELAY_BASE_URL,
  QINGHONG_RELAY_SITE,
} from '../../constants/qinghong-relay';
...
export function SettingsAPI() {
  ...
  const openQinghongRelaySite = () => {
    const api = window.electronAPI;
    if (api?.openExternal) {
      void api.openExternal(QINGHONG_RELAY_SITE);
      return;
    }
    window.open(QINGHONG_RELAY_SITE, '_blank', 'noopener,noreferrer');
  };
  ...
  return (
    <div className="space-y-5">
      {/* Free client + recommended Qinghong relay */}
      <div className="rounded-xl border border-accent/25 bg-accent/5 p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1 space-y-2">
            <div>
              <p className="text-sm font-semibold text-text-primary">{t('api.relay.title')}</p>
              <p className="mt-1 text-sm leading-6 text-text-secondary">{t('api.relay.desc')}</p>
            </div>
            <p className="text-xs text-text-muted">
              {t('api.relay.baseUrlLabel')}{' '}
              <code className="rounded bg-background px-1.5 py-0.5 font-mono text-[11px] text-text-secondary">
                {QINGHONG_RELAY_BASE_URL}
              </code>
            </p>
            <div className="flex flex-wrap gap-2 pt-1">
              ...
              >
                ...
                {t('api.relay.openSite')}
              </button>
              <button
                type="button"
                onClick={applyQinghongRelay}
                className="inline-flex items-center gap-1.5 rounded-lg border border-border bg-background px-3 py-2 text-sm text-text-primary hover:bg-surface-hover"
              >
                {t('api.relay.applyConfig')}
              </button>
            </div>
            <p className="text-xs leading-5 text-text-muted">{t('api.relay.hint')}</p>
          </div>
        </div>
      </div>
