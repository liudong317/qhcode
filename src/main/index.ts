...
app
  ...
  .then(async () => {
    ...
    log('[Runtime] Using QhCode agent SDK for all providers');
    ...
    // Auto-update from Gitee Releases (liudong59/qhcode), not upstream Open Cowork
    if (!isDev) {
  ...
  .catch((error) => {
    ...
    dialog.showErrorBox('QhCode 启动失败', `${message}\n\n请查看日志获取更多信息。`);
    app.quit();
  });
