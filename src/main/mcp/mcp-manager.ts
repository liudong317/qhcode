...
export class MCPManager {
  ...
  private getQinghongFinanceServerPath(): string {
    return this.getMcpServerPath('qinghong-finance-server.ts');
  }

  private getQinghongMarketServerPath(): string {
  ...
  private async connectServerInternal(config: MCPServerConfig): Promise<void> {
    ...
    if (config.type === 'stdio') {
      ...
      const isBuiltinServer =
        ...
        config.name === '晴红资讯' ||
        config.name === 'Qinghong_Finance' ||
        config.name === '晴红行情' ||
        config.name === 'Qinghong_Market';
      const isOldConfig =
      ...
      args = args.map((arg) => {
        ...
        if (arg === '{QINGHONG_FINANCE_SERVER_PATH}') {
          return this.getQinghongFinanceServerPath();
        }
        if (arg === '{QINGHONG_MARKET_SERVER_PATH}') {
          return this.getQinghongMarketServerPath();
        }
        return arg;
      });
